...
class jtag_worker:  # 解析bsdl文件， 基于bsdl从jtag获取数据
    ...
    def transfer_bsdl_to_json(self, isWrite2Local=True):  # 把芯片的bsdl描述文件中的pin信息提取出来，转为json格式
        ...
        ast = main(bsdl_path, '_bsdl_description_')
        # ast = generic_main(main, bsdlParser, name='bsdl')
    ...
    def init_jc(self):  # 初始化jtag设备，获取设备号，给设备添加bsdl文件
        res = self.jc.get_probe_names()
        if self.isPrintLog:
            print(f'检测到jtag探头成功{res}')

        res = self.jc.open_probe(0)
        if self.isPrintLog:
            print(f'打开jtag探头: {res}')
        ...
        for i in range(numDevs):
            bsdl_dev_id = str(hex(self.jc.get_bsdl_id(bsdl_path)))[2:]
            if self.isPrintLog:
            ...
            if bsdl_dev_id in dev_id:  # 设备名称和bsdl名称一致时，给设备加载bsdl文件
                self.dev_num = i
                self.jc.bsdl_attach(bsdl_path, i)
                if self.isPrintLog:
